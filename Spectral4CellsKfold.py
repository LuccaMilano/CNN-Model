...
if __name__ == "__main__":
    ...
    output = tf.keras.layers.Dense(54, activation='relu')(combined) 
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(delta_wave)):
        ...
        concatenated_model.fit(
            x=[delta_wave_train, theta_wave_train, alpha_wave_train, beta_wave_train],
            y=labels_train,
            batch_size=64,
            epochs=100,
            validation_data=([delta_wave_val, theta_wave_val, alpha_wave_val, beta_wave_val], labels_val)
        )
