...
if __name__ == "__main__":
    ...
    combined = tf.keras.layers.Concatenate()([model1, model2])
    ...
    concatenated_model = tf.keras.Model(inputs=[input, input2], outputs=output,name= 'Concatenated_Model_Frequency')
    ...
    history = concatenated_model.fit(
        x=[delta_wave_train, theta_wave_train],
        y=train_labels,
        batch_size=64,
        epochs=100,
        callbacks=[callback],
        validation_data=([delta_wave_val, theta_wave_val], val_labels)
    )
    ...
    test_loss, test_accuracy = concatenated_model.evaluate([delta_wave_test, theta_wave_test], test_labels)

    predictions = concatenated_model.predict([delta_wave_test, theta_wave_test])
