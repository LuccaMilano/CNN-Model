...
if __name__ == "__main__":
    ...
    combined = tf.keras.layers.Concatenate()([model1, model2, model3, model4])
    ...
    concatenated_model = tf.keras.Model(inputs=[input, input2, input3, input4], outputs=output,name= 'Concatenated_Model_Frequency')
    ...
    history = concatenated_model.fit(
        x=[theta_wave_train, alpha_wave_train, beta_wave_train, gamma_wave_train],
        y=train_labels,
        batch_size=64,
        epochs=100,
        callbacks=[callback],
        validation_data=([theta_wave_val, alpha_wave_val, beta_wave_val, gamma_wave_val], val_labels)
    )
    ...
    test_loss, test_accuracy = concatenated_model.evaluate([theta_wave_test, alpha_wave_test, beta_wave_test, gamma_wave_test], test_labels)

    predictions = concatenated_model.predict([theta_wave_test, alpha_wave_test, beta_wave_test, gamma_wave_test])
